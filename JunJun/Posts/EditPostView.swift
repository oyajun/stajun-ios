...
/// Sheet for editing an existing study-time post.
struct EditPostView: View {
    let post: Post
    /// Called with the updated study post after a successful submission.
    var onUpdated: ((StudyPost) -> Void)?
    ...
    init(post: Post, onUpdated: ((StudyPost) -> Void)? = nil) {
        self.post = post
        self.onUpdated = onUpdated
        let clamped = min(max(post.minutes, 0), 1440)
        _hours = State(initialValue: clamped / 60)
        _mins = State(initialValue: clamped % 60)
        _comment = State(initialValue: post.comment ?? "")
    }
    ...
    private func submit() async {
        ...
        do {
            let updated = try await APIClient.updatePost(
                id: post.id,
